...
| `jsonparser/` | 轻量 JSON 字段提取工具，用于 HTTP 返回解析 |
...
3. 需要 JSON 字段提取时读 `jsonparser/jsonparser.md`。
4. 需要摘要或分组加密时先读 `md5/md5.md`、`aes/aes.md`。
5. 需要 Cortex-M fault 现场采集时读 `trace/trace.md`。
6. 需要滤波或数值算法时再进入对应目录主文档。
