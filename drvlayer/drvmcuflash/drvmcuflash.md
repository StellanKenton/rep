...
`drvmcuflash` 提供 MCU 片内 Flash 的最小稳定写存储接口。它对上暴露“逻辑区域 + 偏移”的访问方式，避免业务层直接拿绝对地址擦写程序区；对下只依赖 port/BSP 提供的 sector 查询、擦除和编程钩子。
...
- `DRVMCUFLASH_AREA_USER_START_ADDR = 0x080E0000`
- `DRVMCUFLASH_AREA_USER_SIZE = 0x00020000`

这对应 GD32F407VG 1MB Flash 的最后一个 128KB sector。之所以默认这样配置，是为了在不改业务层接口的前提下，尽量降低误擦程序区的风险。
