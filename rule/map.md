...
| `sys/` | 通用服务型状态机、生命周期与项目绑定 hook | `sys/sys.md` |
| `lib/` | 第三方库与轻量公共库源码归档 | `lib/lib.md` |
| `sys/vfs/` | 统一挂载点与文件语义的轻量 VFS 服务 | `sys/vfs/vfs.md` |
| `sys/log/` | console 与 log 公共 contract | `sys/log/console.md`、`sys/log/log.md` |
...
| 新增或修改公共服务 | `rule/rule.md`、`sys/sys.md` | 对应 sys 主文档、`*_port.h`、`.h/.c` |
| 修改文件系统抽象或挂载分发 | `rule/rule.md`、`sys/sys.md` | `sys/vfs/vfs.md`、`User/port/README.md`、对应 `.h/.c` |
| 新增或迁移第三方库 | `rule/rule.md`、`lib/lib.md` | 对应库目录主文档、库头源文件、项目绑定层 |
| 修改服务生命周期或系统编排 | `example/example.md` | `example/manager/manager.md`、`example/system/system.md`、对应 sys 文档 |
| 修改 console / log | `sys/log/console.md`、`sys/log/log.md` | 对应头文件和实现 |
...
| `sys/xxx` 目录 | `sys/sys.md` + 对应服务主文档 | 状态机语义、port hook、项目绑定边界 |
| `sys/vfs` 目录 | `sys/sys.md` + `sys/vfs/vfs.md` | 挂载点 contract、backend 边界、项目 block device 绑定 |
