...
- 叶子目录主文档必须包含固定章节，并补齐两张 contract 表：
	...
	- `公共函数使用契约表`
- 只要当前目录存在 `port` / provider / platform binding 文件，主文档必须把这些文件显式写进 `port_files`，并在“目录内文件职责”中逐个说明作用，不能只写 core 文件。
- 只要当前目录存在 `port` / provider / platform binding 文件，主文档必须额外补齐 4 类可落地信息：
	- 一个最小可编译的 `port.c` 或 provider 骨架
	- 一张“必填字段 / 必填 hook / 必填 ops 成员”检查表
	- 一组多实例、多 `protocolId`、多 linkId 或多设备映射时的分发规则
	- 一句清楚说明未知配置命中时应如何失败返回或保持零值返回的规则
...
8. 如果该目录依赖 `port` / provider，维护者能否只看 md 就写出最小 `port.h/.c` 骨架。
9. 如果该目录支持多协议、多设备或多映射，维护者能否只看 md 就写出正确的分发分支。
...
- 除 `User/port/rtos_port.*` 外，项目层与 `rep/` 复用层禁止直接调用原生 RTOS API；需要延时、tick、任务创建、统计初始化、互斥或队列能力时，统一通过 `rep/sys/rtos/rtos.h` 暴露的 `repRtos*` 接口访问。
...
- 如果当前是 /init，则先看看脚本文件，给程序添加项目的开发环境，如果是stm32、gd32则添加开发环境参考rep\scripts下的脚本，并依照log文件的要求创建bsp_rtt和rttport文件加入编译，以及参照rep\tools\trace添加纠错程序；如果是esp32则只添加log和console文件，并依照log文件的要求创建bsp_rtt和rttport文件加入编译。
...
- RTT在agent自己使用来查看结果时，必须先找出当前rtt有没有其他进程占用，有就先关闭掉占用的进程，避免出现rtt被占用无法查看结果的情况。直接用RTT的终端不好用最好用一个一次性的 PowerShell TCP 读取直接拉取 RTT 缓冲。在rtt服务启动后可能没有日志输出，有可能是rtt启动时，固件已经不再发送日志了，这时可以通过重启固件来解决这个问题。
